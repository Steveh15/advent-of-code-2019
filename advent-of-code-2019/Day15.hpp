...
namespace Day15 {
	...
	void solution() {
		...
		// Perform search to find oxygen system
		std::map<int, std::vector<std::vector<int>>> path_map;

		// Manually build map of valid initial directions
		for (int i = 1; i < 5; i++) {
			comp1.run(i);
			if (comp1.getOuput() == 1) {
				comp1.run(backward(i));
				path_map[1].push_back({ i });
			}
		}

		int i = 2;
		bool oxygen_found = false;
		std::vector<int> path_to_oxygen;
		while (!oxygen_found) {
			//Loop through all the valid paths of length i - 1
			for (auto valid_path : path_map[i - 1]) {

				// Perform path
				for (int dir : valid_path) comp1.run(dir);

				// For each path, try moving in each direction apart from backward, and if it's valid add it to the valid paths.
				for (int guess_dir = 1; guess_dir <= 4; guess_dir++) {
					if (guess_dir != backward(valid_path.back())) {
						comp1.run(guess_dir);

						if (comp1.getOuput() == 1) {
							// Add to valid paths
							std::vector<int> new_path = valid_path;
							new_path.push_back(guess_dir);
							path_map[i].push_back(new_path);

							// Step back;
							comp1.run(backward(guess_dir));
						}
						else if (comp1.getOuput() == 2) {
							std::cout << "Part 1 solution:  " << i << "\n";
							oxygen_found = true;
							path_to_oxygen = valid_path;
							path_to_oxygen.push_back(guess_dir);
						}
					}
				}
				// Traverse Back to start if oxygen not found
				for (auto it = valid_path.rbegin(); it != valid_path.rend(); ++it) comp1.run(backward(*it));
			}
			i++;
		}


		// Perform another search to find maximum depth
		path_map.clear();

		// Go to oxygen
		for (int dir : path_to_oxygen) comp1.run(dir);

		// Get valid initial directions
		for (int i = 1; i < 5; i++) {
			comp1.run(i);
			if (comp1.getOuput() == 1) {
				comp1.run(backward(i));
		...
		i = 2;
		bool maximum_depth_found = false;
		while (!maximum_depth_found) {
			...
			for (auto valid_path : path_map[i - 1]) {
				...
				for (int guess_dir = 1; guess_dir <= 4; guess_dir++) {
					if (guess_dir != backward(valid_path.back())) {
						...
						if (comp1.getOuput() == 1 || comp1.getOuput() == 2) {
							// Add to valid paths
							std::vector<int> new_path = valid_path;
							new_path.push_back(guess_dir);
							path_map[i].push_back(new_path);
				...
				// Traverse Back to start if maximum depth not found
				if (!maximum_depth_found) for (auto it = valid_path.rbegin(); it != valid_path.rend(); ++it) comp1.run(backward(*it));
			}

			// If no new valid paths found then we've reached maximum depth
			if (path_map[i].size() == 0) {
				maximum_depth_found = true;
				std::cout << "Part 2 solution:  " << i-1 << "\n";
			}
			i++;
		}
